...
export class CsiSimulator {
  ...
  async connectLive(url) {
    // Hack-Safe static mirror: when the full RuView sensing backend is not
    // running, treat same-origin /ws/sensing as a clean demo connection instead
    // of surfacing browser connection errors. This does NOT mark frames live;
    // mode stays 'demo' until _handleLiveFrame verifies real CSI.
    try {
      const u = new URL(url, window.location.href);
      if (u.pathname === '/ws/sensing' && u.hostname === window.location.hostname && u.port === window.location.port) {
        this.socketOpen = true;
        this.verifiedFrame = false;
        return true;
      }
    } catch { /* fall through to WebSocket attempt */ }
