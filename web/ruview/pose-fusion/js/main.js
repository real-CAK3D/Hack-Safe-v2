...
function init() {
  ...
  const defaultWsUrl = `${window.location.protocol === 'https:' ? 'wss' : 'ws'}://${window.location.host}/ws/sensing`;
  ...
  withWsTicket(defaultWsUrl).then(u => csiSimulator.connectLive(u)).then(ok => {
    if (ok) {
      ...
      if (connectWsBtn) connectWsBtn.textContent = '✓ Demo Connected';
    }
  });
...
async function startCamera() {
  ...
  if (ok) {
    ...
    cameraPrompt.querySelector('p').textContent = 'Camera unavailable from this HTTP/Tailscale page. Use CSI Only here, or open http://127.0.0.1:8765/ruview/pose-fusion.html on the Pi for browser webcam permission.';
  }
}
