...
function geoToMapPercent(lat, lon){ lat=Number(lat); lon=Number(lon); const x=2+((lon+180)/360)*96; const merc=Math.log(Math.tan(Math.PI/4+(lat*Math.PI/180)/2)); const y=11+(0.5-(merc/(2*Math.PI)))*66; return [clamp(x,2,98), clamp(y,8,82)]; }
const MAP_ANCHORS={
  'US-ME':[24.8,31.6],'US-NY':[23.2,34.1],'US-FL':[22.8,45.4],'US-TX':[17.0,42.4],'US-CA':[10.2,38.5],'US-WA':[10.5,29.2],
  'UK':[47.0,29.4],'NL':[49.1,31.0],'DE':[50.5,32.5],'CH':[49.5,36.0],'PL':[53.2,31.8],'SE':[52.5,22.4],'IT':[51.7,41.5],'ES':[46.0,42.0],
  'TR':[57.8,42.0],'IN':[69.5,50.0],'SG':[78.0,62.0],'JP':[86.3,41.3],'AU':[83.4,74.0],'BR':[36.2,66.0],'AR':[34.8,80.0],'ZA':[53.5,76.0]
};
function mapAnchorPercent(marker){ return MAP_ANCHORS[marker.name] || geoToMapPercent(marker.lat, marker.lon); }
...
function scheduleCamera(s){
  ...
  cameraTimer=null;
  if(shouldRun){ refreshCameraFrame(true); cameraTimer=setInterval(()=>refreshCameraFrame(false), enabled ? (activeCameraFeed==='local'?450:900) : 1800); }
}
...
function renderGlobalMap(s){
  ...
  const matchesProfile=(m,n)=>{ const nn=norm(n); return nn && ([m.name,...(m.aliases||[])].some(a=>nn.includes(norm(a))||norm(a).includes(nn))); };
  ...
  const markerHtml=markers.map((m,i)=>{ const sel=matchesProfile(m, selectedName); const conn=i===connectedMarkerIndex && (vpnOn||activeName); const [mx,my]=mapAnchorPercent(m); return `<button class="proton-pin ${conn?'connected':''} ${sel?'selected':''}" style="left:${mx.toFixed(2)}%;top:${my.toFixed(2)}%" title="${m.name}" aria-label="${m.name}" onclick="selectProtonProfile(decodeURIComponent('${enc(m.name)}'), false)"><span></span></button>`; }).join('');
...
function renderSystem(sys){
  ...
  document.getElementById('systemViz').innerHTML = `<div class="sec-summary ${posture.toLowerCase()}"><b>SYSTEM POSTURE ${posture}</b><span>${escapeHtml(summary)}</span></div><div class="metric-grid">${metricCell('HOST', sys.hostname||'n/a', (sys.ips||[]).find(ip=>ip.startsWith('100.'))||(sys.ips||[])[0]||'no ip')}${metricCell('UPTIME', uptimeText(sys.uptime_s), sys.load||'load n/a')}${metricCell('THERMAL', cpu?`${cpuF.toFixed(1)}°F`:'n/a', cpu?`${cpu.toFixed?cpu.toFixed(1):cpu}°C`:'')}${metricCell('FAN', fan.available?`${fan.rpm||0} RPM`:'n/a', fan.available?`state ${fan.cooling_state ?? '?'} / ${fan.cooling_max ?? '?'}`:'')}</div>${liveWave('CPU TEMP', cpuHist, `${cpu?cpuF.toFixed(1):'n/a'}°F`, cpu>=75?'var(--red)':cpu>=65?'var(--yellow)':'var(--green)')}${liveWave('RAM PRESSURE', ramHist, mem.text||'n/a', ram>=85?'var(--red)':'var(--cyan)')}${liveWave('DISK PRESSURE', diskHist, disk+'%', disk>=90?'var(--red)':disk>=80?'var(--yellow)':'var(--green)')}${liveWave('LOAD AVG', loadHist, sys.load||'n/a', loadPct>=80?'var(--red)':loadPct>=55?'var(--yellow)':'var(--purple)')}${liveWave('NET RX/TX PULSE', rxHist.map((v,i)=>Math.max(v, txHist[i]||0)), `↓${bytesPerSec(net.rx_bps)} ↑${bytesPerSec(net.tx_bps)}`, 'var(--blue)')}`;
